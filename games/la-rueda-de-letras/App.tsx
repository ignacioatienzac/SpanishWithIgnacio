...
import { generateCrosswordLogic, normalize } from './utils';
...
function App() {
    ...
    const [lastSolvedIds, setLastSolvedIds] = useState<string[]>([]);
    ...
    return (
        <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-800 relative">
            ...
            {flyingLetters.map((item) => (
                <div
                    key={item.id}
                    className="fixed z-50 flex items-center justify-center bg-[#c0392b] text-white rounded-full font-bold shadow-lg pointer-events-none"
                    style={{
                        width: '40px', 
                        height: '40px',
                        left: 0,
            ...
            {/* Game Title / Controls within Game Section */}
            <div className="container mx-auto px-4 py-6 flex flex-wrap justify-between items-center gap-4">
                 <div className="flex items-center gap-2">
                    <h2 className="text-xl md:text-2xl font-bold tracking-tight text-slate-700">{gameTitle}</h2>
                 </div>
            ...
            <main className="flex-1 overflow-y-auto w-full max-w-6xl mx-auto p-4 flex flex-col md:flex-row gap-8 items-start justify-center">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center w-full text-red-500">Error al generar. Intenta de nuevo.</div>
                ) : (
                    <>
                        {/* Left Column: Grid */}
                        <div className="flex-1 flex flex-col items-center w-full">
                            ...
                            {allSolved && (
                                <div className="mt-6 bg-green-100 text-green-800 px-6 py-4 rounded-xl border border-green-200 shadow-sm text-center animate-bounce">
                                    ...
                                    <div className="p-4 max-h-60 overflow-y-auto space-y-4">
                                        <div>
                                            ...
                                            <ul className="space-y-2">
                                                {gameState.words.filter(w => w.dir === 'H').map((w, i) => {
                                                     const wordId = `${w.dir}-${w.x}-${w.y}-${w.wordObj.palabra}`;
                                                     const isSolved = solvedWords.has(wordId);
                                                     const globalIndex = gameState.words.indexOf(w) + 1;
                                                     return (
                                                        <li key={i} className={`text-sm ${isSolved ? 'text-green-600 line-through decoration-2' : 'text-slate-700'}`}>
                                                            <span className="inline-block bg-slate-200 text-slate-600 text-xs font-bold rounded px-1.5 mr-2">{globalIndex}</span>
                                                            {w.wordObj.traduccion_ingles}
                                                        </li>
                                                     );
                                                })}
                                            </ul>
                                        ...
                                        <div>
                                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Verticales</h3>
                                            <ul className="space-y-2">
                                                {gameState.words.filter(w => w.dir === 'V').map((w, i) => {
                                                     const wordId = `${w.dir}-${w.x}-${w.y}-${w.wordObj.palabra}`;
                                                     const isSolved = solvedWords.has(wordId);
                                                     const globalIndex = gameState.words.indexOf(w) + 1;
                                                     return (
